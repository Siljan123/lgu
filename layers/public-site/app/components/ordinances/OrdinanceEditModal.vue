...
import type { LegalDocument, UpdateLegalDocumentPayload, DocumentType, DocumentStatus } from '../../../types/ordinance'
...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'submit', payload: UpdateLegalDocumentPayload): void
}>()
  ...
  ([isOpen, doc]) => {
    if (isOpen && doc) {
      ...
      form.value = {
        ...
        pdf_url: doc.pdf_url,
        date_issued: doc.date_issued ? doc.date_issued.slice(0, 10) : '',
        status: doc.status,
        tags: [...(doc.tags || [])],
      }
    }
...
async function handleFileChange(e: Event) {
  ...
  try {
    ...
    if (res?.publicUrl) {
      ...
      uploadSuccessMessage.value = `Replaced with "${file.name}" successfully!`
    }
  } catch (err: any) {
    console.error('Failed to upload replacement PDF file:', err)
    alert(err?.data?.statusMessage || 'Failed to upload PDF file.')
    selectedPdfFile.value = null
  } finally {
    isUploadingPdf.value = false
  }
...
<template>
  ...
  >
    <div class="bg-white dark:bg-[#1c1c1c] border border-[#dfdfdf] dark:border-[#333333] rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden my-auto animate-in zoom-in-95 duration-200">
      <!-- Modal Header -->
        ...
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
              Update metadata, status, or official PDF copy
            </p>
          </div>
        </div>
        <button
      ...
      <form @submit.prevent="handleSubmit" class="p-5 sm:p-6 space-y-4 sm:space-y-5 max-h-[calc(88vh-80px)] overflow-y-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div class="space-y-1.5">
            ...
            <input
              ...
              required
              placeholder="e.g. Municipal Ordinance No. 2026-001"
        ...
        <div class="space-y-1.5">
          <label class="block text-xs sm:text-sm font-semibold text-neutral-800 dark:text-neutral-200">
            6. Summary / Statement of Purpose (Optional)
          </label>
          <textarea
            v-model="form.description"
            rows="3"
            placeholder="Brief explanation of policy, scope, or background context..."
            class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-xl text-neutral-900 dark:text-neutral-100 focus:outline-hidden focus:border-[#dc2626] focus:ring-2 focus:ring-[#dc2626]/10 transition"
          />
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="flex items-center space-x-2">
            <input
              ...
              placeholder="Type tag (e.g. Environment, Revenue, Health) and press Enter"
              class="flex-1 px-3.5 py-2 text-xs sm:text-sm bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-xl text-neutral-900 dark:text-neutral-100 focus:outline-hidden focus:border-[#dc2626] transition"
            />
            <button
              type="button"
              @click="addTag"
              class="px-3 py-2 text-xs font-semibold bg-neutral-100 dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-800 dark:text-neutral-200 rounded-xl transition cursor-pointer flex items-center space-x-1"
            >
              <Plus class="size-3.5" />
              <span>Add</span>
            </button>
        ...
        <div class="space-y-2 p-4 bg-neutral-50 dark:bg-neutral-900/80 border border-neutral-200 dark:border-neutral-800 rounded-xl">
          <label class="block text-xs sm:text-sm font-semibold text-neutral-800 dark:text-neutral-200">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center gap-3">
            <label class="inline-flex items-center space-x-2 px-4 py-2.5 text-xs font-semibold bg-white dark:bg-[#1c1c1c] border border-neutral-200 dark:border-neutral-700 hover:border-[#dc2626] text-neutral-800 dark:text-neutral-200 rounded-xl cursor-pointer shadow-xs transition shrink-0">
          ...
          <div class="space-y-1 pt-1">
            <span class="text-[11px] text-neutral-500 dark:text-neutral-400 block">
              Current / Target PDF URL:
            </span>
            <input
              type="text"
              ...
              class="w-full px-3.5 py-2 text-xs bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-xl text-neutral-900 dark:text-neutral-100 focus:outline-hidden focus:border-[#dc2626] transition font-mono"
            />
          </div>
        </div>
